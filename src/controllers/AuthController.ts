...
export class AuthController {
  ...
  static register: ExpressHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!errors.isEmpty()) {
        debugLog("REGISTER - Validation Errors", errors.array());
      ...
      if (!dbConnection.isInitialized) {
        debugLog("REGISTER - Database Connection", "Initializing database connection");
      ...
      debugLog("REGISTER - Extracted Data", {
        lastName,
        firstName,
        telephone,
        email,
      ...
      if (role === UserRole.ADMIN) {
        debugLog("REGISTER - Admin Check", "Checking for existing admin");
        const existingAdmin = await userRepository.findOne({
          where: { role: UserRole.ADMIN },
        })

        if (existingAdmin) {
          debugLog("REGISTER - Admin Check", "Admin already exists");
          res.status(400).json({
      ...
      debugLog("REGISTER - Email Check", `Checking if email ${email} exists`);
      const existingUser = await userRepository.findOne({
        where: { email },
      })

      if (existingUser) {
        debugLog("REGISTER - Email Check", "Email already exists");
        res.status(400).json({
      ...
      debugLog("REGISTER - Username Generation", { baseUsername, finalUsername: username });
      ...
      debugLog("REGISTER - User Data for Creation", { ...userData, password: "[HIDDEN]" });

      const user = userRepository.create(userData)

      debugLog("REGISTER - User Entity Created", "User entity created successfully");

      await userRepository.save(user)

      debugLog("REGISTER - User Saved", "User saved to database successfully");
  ...
  static registerEmployee: ExpressHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
      // Debug: Log incoming request
      ...
      const requiredFields = { lastName, firstName, telephone, email };
